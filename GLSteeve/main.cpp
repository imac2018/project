...
int setShader(GLuint shaderID, string shaderFilename){
	int length, maxlength = 200;
	...
	try{
		file.open("vertex.vs.glsl", ios::in);
	}
	catch (const ifstream::failure& e){
		cerr << string("Vertex Shader File cannot be opened\n") + string(e.what());
		return 0;
	}

	vShaderFile.seekg(0, std::ios::end);
	content.reserve(vShaderFile.tellg());
	vShaderFile.seekg(0, std::ios::beg);

	content.assign(std::istreambuf_iterator<char>(vShaderFile), std::istreambuf_iterator<char>());
	tabchar[0] = new char[content.length()*sizeof(char)+1]();
	strcpy(tabchar[0], content.c_str());
	tabint[0] = content.length();

	glShaderSource(shaderID, 1, tabchar, tabint);
