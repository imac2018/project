...
ColliderObject *ColliderObject::lightTranslate(float tx, float ty) const
{
	return new ColliderCircle(_radius, _center + QPointF(tx,ty));
}

void ColliderObject::translate(const QPointF &t)
{
	translate(t.x(),t.y());
}

ColliderObject *ColliderObject::lightTranslate(const QPointF &t) const
{
	return ColliderObject::lightTranslate(t.x(),t.y());
}
...
{
	const ColliderCircle* c;
	const QList<QPointF>* points;
	QList<QPointF>::ConstIterator i, end;
	float r;
	int ret1, ret2;
	QLineF line;
	const QRectF* bounds;
	...
	switch(object.type()){
	case Circle:
		c = dynamic_cast<const ColliderCircle*>(&object);
		if(c != NULL){
			r = c->radius() + radius();
		...
		return false;
	case Rect:
		bounds = &(dynamic_cast<const ColliderRect*>(&object)->bounds());
	...
	case Polygone:
		points = &(dynamic_cast<const ColliderPolygon*>(&object)->points());
		if(points->length()<=1)
			return false;
		i = points->constBegin();
		end = points->constEnd() - 1;
		ret1 = ret2 = collisionWithSegment(*i, *(i+1));
		if(ret1 == 1)
			return true;
		i++;
		while(i!=end){
			ret1 = collisionWithSegment(*i, *(i+1));
			if(ret1 == 1)
				return true;
			ret2 &= ret1;
			i++;
...
bool ColliderRect::collisionWithObject(const ColliderObject &object) const
{
	...
	default:
		ret1 = ret2 = object.collisionWithSegment(_bounds.topLeft(),_bounds.topRight());
		if(ret1 == 1)
			return true;
		ret1 = object.collisionWithSegment(_bounds.topRight(),_bounds.bottomRight());
		if(ret1 == 1)
			return true;
		ret2 &= ret1;
		r = object.collisionWithSegment(_bounds.bottomRight(),_bounds.bottomLeft());
		if(ret1 == 1)
			return true;
		ret2 &= ret1;
		ret1 = object.collisionWithSegment(_bounds.bottomLeft(),_bounds.topLeft());
		return (ret1==1) || ((ret1&ret2)==2);
	}
	return false;
}
...
ColliderPolygon::ColliderPolygon(float radius, QPointF center)
	: ColliderObject(Polygone, radius, center)
{
...
ColliderPolygon::ColliderPolygon(float radius, QPointF center,
								 const QList<QPointF> &points)
	: ColliderPolygon(radius, center)
{
	_points = points;
}

void ColliderPolygon::addPoint(QPointF point)
{
	_points.append(point);
}

bool ColliderPolygon::collisionWithObject(const ColliderObject &object) const
{
	...
	case Line:
		...
		return collisionWithSegment(line.p1(),line.p2()) > 0;
	default:
		i = _points.constBegin(), end = _points.constEnd() - 1;
		ret1 = ret2 = object.collisionWithSegment(*i,*(i+1));
		if(ret1 == 1)
			return true;
		i++;
		while(i!=end){
			ret1 = object.collisionWithSegment(*i,*(i+1));
			if(ret1 == 1)
				return true;
			ret2 &= ret1;
			i++;
...
bool ColliderPolygon::collisionWithPoint(const QPointF &A) const
{
	QList<QPointF>::ConstIterator i = points().constBegin(),
									end = points().constEnd()-1;
	while(i!=end){
		if(! pointOnTheRightOf(A,(*i),*(i+1)))
			return false;
	}
	return true;
}

int ColliderPolygon::collisionWithSegment(const QPointF &A, const QPointF &B) const
{
	if(_points.length()<=1)
		return 0;
	int ret1, ret2;
	QList<QPointF>::ConstIterator i = _points.constBegin(),
									end = _points.constEnd()-1;

	ret1 = ret2 = intersectionBetweenSegment(A,B,(*i),*(i+1));
	if(ret1 == 1)
		return true;
	i++;
	while(i!=end){
		ret1 = intersectionBetweenSegment(A,B,(*i),*(i+1));
		if(ret1 == 1)
			return true;
		ret2 &= ret1;
		i++;
...
void ColliderPolygon::translate(float tx, float ty)
{
	QList<QPointF>::iterator i = _points.begin(),
								end = _points.end();
	QPointF t(tx,ty);
	while(i!=_points.end()){
		(*i) += t;
...
const QList<QPointF>& ColliderPolygon::points() const
{
	return _points;
}
