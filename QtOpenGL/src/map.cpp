		...
		for(int i=0;i<models.size();i++){
			if(models[i].length()){
				foreach(Object3D* o, models[i]){
                    o->diffuseColor = QColor(200,200,200);
                    o->specularColor = QColor(30,30,30);
				}
			}
		}
...
float Map::initializeMapObjects()
{
	...
	for(;icase<end;icase++){
		...
		if(cases[x][y]->type() == Case::Previous){
			pos = caseCenter(x,y);
            foreach(Object3D* object, models[Stairs]){
                o = new Object3D(*object);
				o->transformM.translate(pos);
				QMatrix4x4 rotater;
				switch (_initialOrientation) {
				case EAST:
                    rotater.rotate(90,0,1,0);
                    break;
				case WEST:
                    rotater.rotate(270,0,1,0);
					break;
				case SOUTH:
                    rotater.rotate(180,0,1,0);
					break;
				}
...
bool Map::validCase(QPoint p) const
{
