...
Button::Button(Renderer& r, QString title, QRectF bounds,
    ...
    : GuiElement(Object3D::makeRect(bounds), Qt::white, GL_TRIANGLE_STRIP),
                title(title), pressed(false), hover(false), bounds(bounds)
{
...
Button::Button(Renderer &r, QString btnImagePath, QRectF bounds)
    : GuiElement(Object3D::makeRect(bounds),
...
GuiElement::GuiElement(std::vector<Vertex3D> vertex, const QColor &globalColor, int drawMode)
    : Object3D(vertex, globalColor, drawMode), invisible(false)
{}
...
DialogFrame::DialogFrame(Renderer &r, QString title, QRectF bounds, int fontsize)
    : GuiElement(Object3D::makeRect(QRectF(-1,-1,2,2)), Qt::white, GL_TRIANGLE_STRIP),
      bounds(bounds)
{
	QPixmap text(720,720*bounds.height() / bounds.width());
	text.fill(Qt::transparent);
