								   ...
								   QColor color, float scale)
{
    ...
    vertex << Vertex3D(-scale,-scale,0,color.redF(),color.greenF(),color.blueF())
									  ...
									  float scaleX, float scaleY, float repeat)
{
	...
	if(isZero(scaleY))
		scaleY = scaleX;
    QList<Vertex3D> vertex;
    vertex << Vertex3D(-scaleX,-scaleY,0,repeat,repeat)
           << Vertex3D(-scaleX,scaleY,0,repeat,0)
           << Vertex3D(scaleX,-scaleY,0,0,repeat)
           << Vertex3D(scaleX,scaleY,0,0,0);
	QList<Vertex3D>::iterator i;
	QMatrix4x4 finalRotationMat;
	finalRotationMat.rotate(rotationX,1,0,0);
	finalRotationMat.rotate(rotationY,0,1,0);
